    ...
    <div class="box">
        <div class="top">
            <div class="top-header">
                ...
                <span>订单号: {{orderInfo.id}}
                    <span class="line"></span>
                    支付方式: 在线支付
                    <span class="line"></span>
                    订单创建时间: {{formatTime(orderInfo.createTime)}}
                    <span class="line"></span>
                    订单确认人员: {{orderInfo.acceptionOfficer}}
                </span>
            </div>
            <div class="top-body">
                <div class="infoBox">
        ...
        <div class="bottom">
            <div class="cost">
                <span>租金: {{carInfo.rentPrice}}元</span>
                <span class="total">共计: 10000元</span>
            </div>
            <div class="btn-group">
                <div class="btn-item">
                    <a-button v-if="orderInfo.status === 2" type="primary" @click="confirmRentout">确认出车</a-button>
                </div>
                <div class="btn-item">
                    <a-button v-if="orderInfo.status === 3" type="danger" @click="cancelOrder">确认还车</a-button>
                </div>
            </div>
        </div>
    </div>
            ...
            confirmRentout() {
                ...
                updateOrder(params).then(res => {
                    if (res.status === 200) {
                        ...
                        this.$router.push({path: "/carRentOut"})
                    }
                })
            },
            cancelOrder() {
